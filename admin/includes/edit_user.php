...
//        $hashed_password = crypt($user_password, $salt);
    ...
    // Check the user password is empty or not
    if (!empty($user_password)) {
        ...
        $user_row = mysqli_fetch_array($get_user_query);
        $db_user_password = $row['user_password'];

        // Password HASH
        if ($db_user_password != $user_password) {
            $hashed_password = password_hash($user_password, PASSWORD_BCRYPT, array('cost' => 15));
        }
